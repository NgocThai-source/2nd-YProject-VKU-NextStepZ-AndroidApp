...
const getImageUrl = (imagePath: string): string => {
  if (!imagePath) return '';
  // If it's already a full URL (http/https), a data URL, or a blob URL, return as is
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                ...
                {post.updatedAt && post.updatedAt !== post.timestamp && (
                  <span className="ml-1 text-gray-500/80" title={`Đã chỉnh sửa: ${new Date(post.updatedAt).toLocaleString('vi-VN')}`}>
                    • (đã chỉnh sửa)
                  </span>
                )}
              </p>
